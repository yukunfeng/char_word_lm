...
def verbose_test(idx2word, counter, word_indexs, scores):
    ...
    for row_i, row in enumerate(word_indexs, 0):
        ...
        for col_i, word_index in enumerate(row, 0):
            ...
            if word in counter:
                freq = counter[word]
                if freq < 15:
                  interval_loss[0] += score
                  interval_count[0] += 1
                elif freq < 30:
                  interval_loss[1] += score
                  interval_count[1] += 1
                elif freq < 45:
                  interval_loss[2] += score
                  interval_count[2] += 1
                elif freq < 60:
                  interval_loss[3] += score
                  interval_count[3] += 1
                elif freq < 75:
                  interval_loss[4] += score
                  interval_count[4] += 1
                elif freq < 90:
                  interval_loss[5] += score
                  interval_count[5] += 1
                else:
                  interval_loss[5] += score
                  interval_count[5] += 1
    ...
    return print_contents, interval_count, interval_loss
...
def evaluate(
    ...
    total_loss = 0.
    # 1-15, 15-30, 30-45, 45-60, 60-75, 75-90, 90+
    interval_total_loss = [0] * 7
    interval_total_count = [0] * 7
    ntokens = len(corpus.dictionary)
    ...
    with torch.no_grad():
        for i in range(0, data_source.size(0) - 1, args.bptt):
            ...
            if verbose:
                ...
                print_contents, interval_count, interval_loss = verbose_test(
                    corpus.dictionary.idx2word, counter,
                    data.t(), verbose_loss.t()
                )
                for i in range(len(interval_count)):
                  interval_total_loss[i] += interval_loss[i]
                  interval_total_count[i] += interval_count[i]
    ...
    if verbose:
        #  fh_out.close()
        interval_res = [0] * 7
        for i in range(len(interval_total_loss)):
          if interval_total_count[i] != 0:
            res = math.exp(interval_total_loss[i] / interval_total_count[i])
            interval_res[i] = res
...
# run logging files
interval_res = evaluate(test_data, fixed_test_data, ngram_test, ngram_test_len, verbose=True)
print(f"1-15, 15-30, 30-45, 45-60, 60-75, 75-90, 90+")
print(f"{interval_res}")
